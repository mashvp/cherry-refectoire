...
const Image = ({field, className, offset, wPos = 0.5, wLength = 0.5, min = -1}:any) => {
  return (
    <ScrollElement
      ...
      min={min}
      >
        <ParrallaxInner field={field} className="aspect-1 mediaCtn w-full"/>
    </ScrollElement>
  )
}
...
const Text = ({field, index, class1, class2, class3,  wPos = 0.5, wLength = 0.15, min = 0}:any) => {
  return (
    <>
      <TextScroll
        // node="h3"
        className={`${class1} hs1 h-min z-10 whitespace-nowrap`}
        windowPosition={wPos}
        windowLength={wLength}
        >
      ...
      <TextScroll
        // node="h3"
        className={`${class2} hs1 h-min z-10 whitespace-nowrap`}
